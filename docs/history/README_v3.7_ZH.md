...
- **Skills 运行元数据**：当前完整 profile 下注册的所有 MCP 工具（最多 12 个）均使用 `ToolResult` 包装结构化 payload，并通过 `meta` 暴露 `tool_name`、`execution_ms`、`success` 及工具特定计数。单连接结果包含 `db_type` 和 `connection_id`；批量诊断使用 `connection_scope="all"` 和汇总计数，不冒用默认连接身份。Mutation 结果还会镜像 `execution_outcome` 和结构化失败的 `error_code`。元数据有意不包含原始 SQL、返回数据行、参数值、DSN、凭据、host 或 SQLite 文件路径。
  ...
  - **最小示例**（完整请求/响应见 [TEST_MCP_CLIENT_GUIDE.md](../guides/TEST_MCP_CLIENT_GUIDE_v3_7.md)）：
...
- 之后，[在(v1.0)版本](../architecture/LLM_TO_MCP_FEASIBILITY_ANALYSIS.md) 为了增加对标准化MCP服务架构的支持，对解决方案进行了重构。同时进行解耦，在增强安全性和可扩展性的同时简化维护。
...
### 项目路线图
**Agent Skills和扩展性**  
**Skills 扩展层已在 v3.0 版本中添加**（2026年3月）。详见 [MCP_AGENTS_SKILLS_DESIGN.md](../architecture/MCP_AGENTS_SKILLS_DESIGN.md)。
...
这就是本项目虽然保留了最初的GEMINI.md，但仅作为记录使用，并且也未增加AGENTS.md的原因。但SKILL.md或类似的"渐进式"文档是良好的实践。本项目的相关文档 [REFACTORING_LOG.md](../../REFACTORING_LOG.md) 和 [PROMPT_ENGINEERING_BEST_PRACTICES.md](../guides/PROMPT_ENGINEERING_BEST_PRACTICES.md) 体现了这一实践。
...
单目标调用省略别名时的实际默认连接及 `check_connection(scope="all")` 的全部配置目标，见[部署验收条件](../guides/MCP_AGENT_BEHAVIOR_VALIDATION_ZH.md#18-限制优先级与配置解读的可复用验收)。
...
[v3.7/v3.7.3 发布说明](../releases/RELEASE_NOTES_v3_7.md)。
...
[v3.7.2 迁移表](../releases/RELEASE_NOTES_v3_7.md#sample-skill-names-and-local-files)。

### 受管单语句 Mutation（2026年9月16日）

本轮补充纳入首次正式发布前的同一 v3.7.3。以下 Skill 作者接口调整仍需迁移，
版本号不变不表示扩展契约完全兼容；详见
[版本范围与兼容性说明](../releases/RELEASE_NOTES_v3_7.md#managed-single-statement-mutation-contract--september-16-2026)。
...
- [当前设计记录](../guides/V3_7_CONNECTION_DIAGNOSTICS_DESIGN_ZH.md)说明迁移、
...
- 评审验证：**675 passed、4 skipped**；十个变更 Python 文件 Pyright 通过。
  ...
  [分阶段 Agent 证据](../validation/v3.7/V3_7_3_LIVE_MCP_TEST_UNIFIED_CONNECTION_DIAGNOSTICS_2026_09_22_ZH.md)
...
- 兼容性及证据边界见 [v3.7.3 发布说明](../releases/RELEASE_NOTES_v3_7.md#v373--connection-routing-and-tool-contract-clarity)。
...
- 完整公共契约、兼容影响、接受边界、测试和经过 review 的一手资料见
  [v3.7.2 发布说明](../releases/RELEASE_NOTES_v3_7.md#v372--write-transactions-and-uncertain-results)。
...
- 新增 [设计风险登记表](../security/DESIGN_RISK_REGISTER_ZH.md) 与 [English version](../security/DESIGN_RISK_REGISTER.md)，长期跟踪已接受、暂缓、拒绝和需要策略决策的设计风险
...
设计详情参见 [MCP_AGENTS_SKILLS_DESIGN.md](../architecture/MCP_AGENTS_SKILLS_DESIGN.md)。
...
详细设计决策、妥协和实现细节请参阅 [SQLITE_ADAPTER_DESIGN.md](../architecture/SQLITE_ADAPTER_DESIGN.md)。变更日志详情请参阅 [REFACTORING_LOG.md](../../REFACTORING_LOG.md)。
...
[中文设计与迁移记录](../guides/V3_7_CONNECTION_DIAGNOSTICS_DESIGN_ZH.md)
（[English](../guides/V3_7_CONNECTION_DIAGNOSTICS_DESIGN.md)）。
...
现有配置升级请参阅[名称迁移表](../releases/RELEASE_NOTES_v3_7.md#sample-skill-names-and-local-files)。
...
目录与连接策略一起审核，详见[技能安全规范](../security/SAFETY.md#11-mutationpy-execution-constraints)。
...
2. 优先编写继承 `ManagedMutationBase` 的具体 `Mutation` 类，声明不可变
   ...
   结果值。[结果保留字段清单](../security/SAFETY.md#11-mutationpy-execution-constraints)
...
详细规范请参阅 [MCP_AGENTS_SKILLS_DESIGN.md](../architecture/MCP_AGENTS_SKILLS_DESIGN.md) 和 [skills/SAFETY.md](../security/SAFETY.md)。
...
完整设计详情、执行流程图和行业最佳实践对齐分析，请参阅 [MCP_AGENTS_SKILLS_DESIGN.md](../architecture/MCP_AGENTS_SKILLS_DESIGN.md)。
...
- [Skills 设计文档](../architecture/MCP_AGENTS_SKILLS_DESIGN.md)：v3.0 Skills 扩展层架构和设计决策
- [Skills 安全策略](../security/SAFETY.md)：面向技能作者的安全治理
- [v3.5-v3.7 命名连接、Skills 与批准流程说明](../guides/V3_5-V3_7_SKILLS_GUIDE_ZH.md)：解释命名连接、Mutation 写策略、preview-token、Skill 连接范围、单 mutation worker 与批准边界
- [v3.5 发布说明](../releases/RELEASE_NOTES_v3_5.md)：命名多连接版本摘要、兼容性、限制和验证证据
- [v3.6/v3.6.1 发布说明](../releases/RELEASE_NOTES_v3_6.md)：Mutation preview-token、命名写策略、execution binding 修复与同进程部署边界定稿
- [v3.7/v3.7.3 发布说明](../releases/RELEASE_NOTES_v3_7.md)：v3.7 能力、事务结论及当前连接路由/工具契约修正
- [设计风险登记表](../security/DESIGN_RISK_REGISTER_ZH.md)：长期维护的设计、安全与运维风险登记
- [可行性分析](../architecture/LLM_TO_MCP_FEASIBILITY_ANALYSIS.md)：LLM 到 MCP 转换的详细分析
- [原始上下文](../../GEMINI.md)：项目背景和开发指南
- [重构日志](../../REFACTORING_LOG.md)：重构变更文档（v2.0 — v3.7.3）
- [MCP 客户端测试指南](../guides/TEST_MCP_CLIENT_GUIDE_v3_7.md)：通过客户端测试 MCP 服务器的指南
- [MCP Agent 编排行为验证方法](../guides/MCP_AGENT_BEHAVIOR_VALIDATION_ZH.md)：验证 Agent 自然工具选择、重复调用、连接路由和渐进披露效果
- [MCP 工具契约与评测指南](../guides/PROMPT_ENGINEERING_BEST_PRACTICES.md)：面向本项目的工具 schema、描述、instructions、安全边界与评测指南
