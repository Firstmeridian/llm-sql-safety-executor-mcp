...
- **Skills Runtime Metadata**: All registered MCP tools in the current full profile (up to 12) wrap their structured payloads in `ToolResult` and expose runtime `meta` fields (`tool_name`, `execution_ms`, `success`, plus tool-specific counters). Single-connection results include `db_type` and `connection_id`; batch diagnostics use `connection_scope="all"` and aggregate counts without a default-connection identity. Mutation results also mirror `execution_outcome` and structured-failure `error_code`. Metadata intentionally excludes raw SQL, returned rows, parameter values, DSNs, credentials, hosts, and SQLite file paths.
  ...
  - **Minimal example** (see [TEST_MCP_CLIENT_GUIDE.md](../guides/TEST_MCP_CLIENT_GUIDE_v3_7.md) for full request/response examples):
...
- Later, [in version (v1.0)](../architecture/LLM_TO_MCP_FEASIBILITY_ANALYSIS.md), the solution was refactored to add support for the standardized MCP service architecture. Decoupling was also performed to simplify maintenance while enhancing security and scalability.
...
- [In version (v2.0)](../../REFACTORING_LOG.md), optimizations for query efficiency and call risks were made for actual MCP usage scenarios.
  ...
  3. Added [examples of multi-Agent calls to this MCP service](README_v3.7.md#autogen-multi-agent-example), based on the AutoGen framework, to demonstrate the combination of Agents and this service.
...
**Skills extension layer added in v3.0** (March 2026). See [MCP_AGENTS_SKILLS_DESIGN.md](../architecture/MCP_AGENTS_SKILLS_DESIGN.md) for details.
...
Of course, unlike standard Agent Skills, this project's Skills provide pre-built, safety-constrained operations that the server executes on the Agent's behalf. This difference is intentional, primarily for [security considerations](README_v3.7.md#skills-design-considerations).
...
*   Configure the environment `cp .env.example .env` and edit .env with your database credentials [Configure environment variables in .env](README_v3.7.md#configuration).
...
[deployment acceptance criteria](../guides/MCP_AGENT_BEHAVIOR_VALIDATION_ZH.md#18-限制优先级与配置解读的可复用验收).
...
[Release Notes v3.7/v3.7.3](../releases/RELEASE_NOTES_v3_7.md).
...
are listed in the [v3.7.2 migration table](../releases/RELEASE_NOTES_v3_7.md#sample-skill-names-and-local-files).
...
[version scope and compatibility note](../releases/RELEASE_NOTES_v3_7.md#managed-single-statement-mutation-contract--september-16-2026).
...
- The [current design record](../guides/V3_7_CONNECTION_DIAGNOSTICS_DESIGN.md)
...
- Review verification: **675 passed, 4 skipped**; ten changed Python files pass
  ...
  named SQLite and all-connection smoke checks. [Staged Agent evidence](../validation/v3.7/V3_7_3_LIVE_MCP_TEST_UNIFIED_CONNECTION_DIAGNOSTICS_2026_09_22_ZH.md)
...
- See the [v3.7.3 notes](../releases/RELEASE_NOTES_v3_7.md#v373--connection-routing-and-tool-contract-clarity)
...
- Full contract, compatibility impact, accepted boundaries, tests, and reviewed
  ...
  [v3.7.2 release notes](../releases/RELEASE_NOTES_v3_7.md#v372--write-transactions-and-uncertain-results).
...
- Added [Design Risk Register](../security/DESIGN_RISK_REGISTER.md) and [中文版本](../security/DESIGN_RISK_REGISTER_ZH.md) as long-term tracking documents for accepted, deferred, rejected, and policy-required design risks
...
For design details, see [MCP_AGENTS_SKILLS_DESIGN.md](../architecture/MCP_AGENTS_SKILLS_DESIGN.md).
...
For detailed design decisions, compromises, and implementation details, see [SQLITE_ADAPTER_DESIGN.md](../architecture/SQLITE_ADAPTER_DESIGN.md). For change log details, see [REFACTORING_LOG.md](../../REFACTORING_LOG.md).
...
behavior. See the [design and migration record](../guides/V3_7_CONNECTION_DIAGNOSTICS_DESIGN.md).
...
See the [name migration table](../releases/RELEASE_NOTES_v3_7.md#sample-skill-names-and-local-files)
...
policies; see [Skills security governance](../security/SAFETY.md#11-mutationpy-execution-constraints).
...
2. Prefer a concrete `Mutation` class inheriting from `ManagedMutationBase`.
   ...
   issuing a token. See [the reserved result fields](../security/SAFETY.md#11-mutationpy-execution-constraints)
...
For full specifications, see [MCP_AGENTS_SKILLS_DESIGN.md](../architecture/MCP_AGENTS_SKILLS_DESIGN.md) and [skills/SAFETY.md](../security/SAFETY.md).
...
For complete design details, execution flow diagrams, and industry best practices alignment analysis, see [MCP_AGENTS_SKILLS_DESIGN.md](../architecture/MCP_AGENTS_SKILLS_DESIGN.md).
...
- [Skills Design](../architecture/MCP_AGENTS_SKILLS_DESIGN.md): v3.0 Skills extension layer architecture and design decisions
- [Skills Security Policy](../security/SAFETY.md): security governance for skill authors
- [Release Notes v3.5](../releases/RELEASE_NOTES_v3_5.md): named multi-connection release summary, compatibility notes, limits, and validation evidence
- [Release Notes v3.6/v3.6.1](../releases/RELEASE_NOTES_v3_6.md): mutation preview tokens, named-write policy, execution binding fixes, and the formalized same-process deployment boundary
- [Release Notes v3.7/v3.7.3](../releases/RELEASE_NOTES_v3_7.md): v3.7 capabilities, transaction outcomes, and current connection-routing/tool-contract corrections
- [v3.5-v3.7 Skills Guide (Chinese)](../guides/V3_5-V3_7_SKILLS_GUIDE_ZH.md): connection routing, write policy, preview tokens, Skill scope, and approval boundaries
- [Design Risk Register](../security/DESIGN_RISK_REGISTER.md): Long-term design, security, and operations risk register
- [Feasibility Analysis](../architecture/LLM_TO_MCP_FEASIBILITY_ANALYSIS.md): Detailed analysis of LLM to MCP conversion
...
- [MCP Client Test Guide](../guides/TEST_MCP_CLIENT_GUIDE_v3_7.md): Guide for testing MCP Server via client
- [MCP Agent Behavior Validation Method (Chinese)](../guides/MCP_AGENT_BEHAVIOR_VALIDATION_ZH.md): Method for validating natural tool selection, redundant calls, connection routing, and progressive disclosure
- [MCP Tool Contract and Evaluation Guide](../guides/PROMPT_ENGINEERING_BEST_PRACTICES.md): Project guidance for tool schemas, descriptions, instructions, safety boundaries, and evaluation
