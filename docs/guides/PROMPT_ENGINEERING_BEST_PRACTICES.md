...
**Implementation alignment reviewed:** September 26, 2026 (v3.8)

Current source locations: UTF-8 [prompt resources](../../src/sql_safety_executor/prompts/),
[tool registration](../../src/sql_safety_executor/mcp/server.py), and code-owned
[annotations/output schemas](../../src/sql_safety_executor/mcp/contracts.py).
Parameter types and defaults stay in Python. Missing registered prompt resources
fail server creation. See the [v3.8 architecture](../architecture/V3_8_IMPLEMENTATION_ZH.md)
for the extraction boundary and [client guide](TEST_MCP_CLIENT_GUIDE.md) for
...
- a packaged tool description or `Annotated`/`Field` parameter description;
...
The retained preview/execute sequence is intentionally deterministic:
...
4. If a response was lost or the token is unknown, consumed, expired, or belongs
   to another process, reconcile any uncertain prior execution before deciding
   whether a new preview and approval are appropriate. Do not infer that a
   missing token proves no write occurred.
...
Optional `request_mutation_approval` expresses the same managed-write boundary
through MRTR. It is registered only when explicitly enabled and requires
MCP `2026-07-28`, form elicitation and a managed single-statement Skill. The
first round returns an immutable review; a continuation with no answer resends
that review without extending its original expiry. Only `accept` with strict
boolean `approve=true` enters shared execution. Decline/cancel closes the
proposal. Do not rerun preview and apply an earlier approval to a changed plan.
The Host is trusted to collect approval; sealed state does not authenticate a
human or replace server-side binding and atomic consumption.

An input-required round is `phase=awaiting_approval`, not business success.
Review, execution and MRTR responses are not cached. Unsupported protocol/form
capability must produce an explicit refusal; the separate preview/execute flow
remains available under its own authorization checks.
...
- MRTR missing answers, strict approval types, decline/cancel, sealed-state
  tampering, review size/expiry, unsupported clients and imperative-Skill refusal.
...
- [v3.8 tests and observed Host limitations](../validation/V3_8_VALIDATION_ZH.md)
- [MCP Agent behavior validation](MCP_AGENT_BEHAVIOR_VALIDATION_ZH.md)
- [v3.6-v3.7 live MCP tests](../validation/v3.7/LIVE_MCP_TEST_V36-V37_ZH.md)
- [Design risk register](../security/DESIGN_RISK_REGISTER.md)
