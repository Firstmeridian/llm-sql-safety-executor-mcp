...
# Keep this syntax aligned with db_adapter.CONNECTION_ID_PATTERN without adding
# a direct loader dependency on the runtime connection module. The loader
# currently imports that module indirectly through sql_safety_checker, so this
# is dependency separation rather than a claim of side-effect-free imports.
...
def _parse_exact_bool(
    ...
    *,
    ...
    if type(value) is not bool:
        raise ValueError(
            f"'{field_name}' must be a YAML boolean in {path}"
        )
    return value
...
def _parse_connection_ids(raw: Any, path: Path) -> list[str]:
    ...
    if not isinstance(raw, list) or not raw:
        raise ValueError(
            f"'connection_ids' must be a non-empty list in {path}"
        )
    ...
    for value in raw:
        ...
        if connection_id in seen:
            raise ValueError(
                f"Duplicate connection_id {connection_id!r} in {path}"
            )
...
def _parse_skill_md(path: Path, dir_name: str) -> SkillMetadata:
    ...
    return SkillMetadata(
        ...
        category=(
            _parse_category(front["category"], path)
...
def _validate_param_schema(raw_params: Any, path: Path) -> dict[str, dict]:
    ...
    for param_name, constraints in raw_params.items():
        ...
        if unknown_constraints:
            unknown = ", ".join(
                sorted(repr(field) for field in unknown_constraints)
            )
            raise ValueError(
