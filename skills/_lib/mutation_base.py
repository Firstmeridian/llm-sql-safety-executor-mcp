...
class ManagedMutationValue:
    ...
    @classmethod
    def from_binding(cls, target: str, key: str | None = None) -> "ManagedMutationValue":
        return cls(
            target=target,
            source="binding",
            key=target if key is None else key,
...
def _validate_managed_value(
    ...
    *,
    ...
    if type(item) is not ManagedMutationValue:
        raise TypeError("Managed mutation values must be ManagedMutationValue instances")
    if not isinstance(item.target, str) or not _MANAGED_NAME_PATTERN.fullmatch(item.target):
    ...
    if item.source in {"params", "binding"}:
        if not isinstance(item.key, str) or not _MANAGED_NAME_PATTERN.fullmatch(item.key):
            raise TypeError("Managed mutation parameter/binding keys must be safe identifiers")
        if item.value is not None:
            raise TypeError("Managed mutation non-constant values cannot define a constant")
    else:
...
def validate_managed_mutation_plan(plan: ManagedMutationPlan) -> ManagedMutationPlan:
    ...
    if len(statements) != 1 or statements[0].get_type().upper() not in _MANAGED_WRITE_TYPES:
        raise TypeError("Managed mutation plans require one direct INSERT, UPDATE, or DELETE statement")
    if isinstance(plan.expected_rowcount, bool) or not isinstance(plan.expected_rowcount, int):
        raise TypeError("Managed mutation expected_rowcount must be a non-negative integer")
...
def resolve_managed_plan_values(
    ...
    *,
    ...
    return (
        _resolve_managed_values(plan.parameters, params=params, execution_binding=execution_binding),
        _resolve_managed_values(plan.result_fields, params=params, execution_binding=execution_binding),
    )
...
def run_managed_mutation(
    *,
    ...
    try:
        ...
        message = "Managed mutation preview state was incomplete; no write was attempted."
        audit_logged = _log_execution_failure(
