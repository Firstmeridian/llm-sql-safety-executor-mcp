...
| DRR-2026-046 | 已接受 | 低 | 2026-08-10 | 仅比较 status 的乐观锁与 ABA 变化 | `update-order-status` 绑定并比较展示的 `status`。外部写入者理论上可在 token 有效期内执行 `pending -> X -> pending`，导致 execute 时 status 相同而中间变化未被发现。 | v3.6.1 不改代码 | 内置状态机无环，无法通过本 Skill 恢复先前状态。加入 revision/`updated_at` 条件需要真实业务 schema 契约和跨数据库 migration，不应为当前不可达路径在框架中虚构通用时间戳。 | 接受为狭窄的外部写入者边界；只要 execute 时 status 与展示值不同，现有保护仍会拒绝写入。 | 真实目标表提供稳定 revision/version，或外部写入者可合法恢复旧状态时，再把该版本字段加入 binding 与 `WHERE`。 |
