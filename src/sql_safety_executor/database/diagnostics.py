...
class ConnectionReport(BaseModel):
    ...
    scope: ConnectionScope = Field(
        description="Requested scope, even when all selects only one configured alias."
    )
    all_connected: bool = Field(
        description="All aliases in this report connected; cleanup health is separate."
    )
    complete: bool = Field(
        description="Every selected alias has a connected or failed result, with none unresolved."
    )
    connection_count: int = Field(
        description="Number of aliases selected for this diagnostic, not necessarily all configured aliases."
    )
    connected_count: int
    cleanup_failed: bool
...
def diagnostic_budget(tool_timeout: float | None) -> float:
    return (
        min(DIAGNOSTIC_BUDGET_SECONDS, 0.8 * tool_timeout)
        if tool_timeout
        else DIAGNOSTIC_BUDGET_SECONDS
    )
...
def probe_connection(config: DatabaseConfig) -> ConnectedCheck | FailedCheck:
    ...
    try:
        ...
        if connected:
            result = ConnectedCheck(
                db_type=config.db_type, status="connected", connected=True
            )
        else:
            # Adapter errors have already passed through the backend sanitizer.
            result = FailedCheck(
                db_type=config.db_type, status="failed", connected=False, error=message
            )
...
class ConnectionDiagnostics:
    ...
    def __init__(
        self, probe: Callable[[DatabaseConfig], ConnectionCheck] = probe_connection
    ):
        self._probe = probe
        self._lock = RLock()
            ...
            if self._executor is None:
                if self._batch_active:
                    raise DiagnosticsUnavailable(
                        "Previous connection diagnostics are still draining."
                    )
                self._executor = ThreadPoolExecutor(
                    max_workers=MAX_DIAGNOSTIC_WORKERS,
                    thread_name_prefix="connection-check",
                )
    ...
    def _work(
        self, config: DatabaseConfig, deadline: float, started: Event
    ) -> _Outcome:
        with self._lock:
        ...
        try:
            ...
            logger.warning(
                "Connection diagnostic worker failed: %s", type(exc).__name__
            )
            result = FailedCheck(
                db_type=config.db_type,
                status="failed",
    ...
    async def run(
        ...
        *,
        ...
        if (
            scope not in ("single", "all")
            or not configs
            or (scope == "single" and len(configs) != 1)
        ):
        ...
        with self._lock:
            if self._executor is None:
                raise DiagnosticsUnavailable(
                    "Connection diagnostics are unavailable while the server is stopped."
                )
            if self._cleanup_failed:
                raise DiagnosticsUnavailable(
            ...
            if self._batch_active:
                raise DiagnosticsUnavailable(
                    "Connection diagnostics are busy. Please retry later."
                )
        ...
        try:
            while next_index < len(configs) or waiting:
                with self._lock:
                    if self._executor is None:
                        raise DiagnosticsUnavailable(
                            "Connection diagnostics stopped during server shutdown."
                        )
                    while (
                        next_index < len(configs)
                        and len(waiting) < MAX_DIAGNOSTIC_WORKERS
                        ...
                        future = self._executor.submit(
                            self._work, config, deadline, started
                        )
            ...
            for config in configs:
                ...
                result: ConnectionCheck = IncompleteCheck(
                    ...
                    connected=None,
                    error=(
                        "Diagnostics stopped after a cleanup failure."
                        if cleanup_failed
                ...
                if entry is not None:
                    ...
                    if started.is_set():
                        result = IncompleteCheck(
                            db_type=config.db_type, status="timeout", connected=None
                        )
            ...
            connected_count = sum(
                result.connected is True for result in results.values()
            )
            return ConnectionReport(
                scope=scope,
                all_connected=connected_count == len(configs),
                complete=all(
                    result.status in ("connected", "failed")
                    for result in results.values()
                ),
                connection_count=len(configs),
                connected_count=connected_count,
                cleanup_failed=cleanup_failed
                or any(item.cleanup_failed for item in results.values()),
                results=results,
            )
        finally:
