...
- Optional connection_ids are syntax-validated and cached as a restrictive
  deployment scope; runtime authorization remains the server's responsibility
- Explicit trusted Skill directories may be outside the project root; resolved
  source paths must remain within each Skill directory. This is not a sandbox.
"""
...
if TYPE_CHECKING:
    from sql_safety_executor.skills.mutation import ManagedMutationPlan, MutationBase

from sql_safety_executor.core.sql import is_sql_safe
...
# Keep this syntax aligned with database.registry.CONNECTION_ID_PATTERN without
# adding a loader dependency on runtime connection registration.
...
class SkillMetadata:
    ...
    # Optional supported DB types (for example ["mysql", "sqlite"]); None = all.
    databases: list[str] | None = None
    # Optional operational profile tags (for example ["demo"]).
    profiles: list[str] = field(default_factory=list)
    tables: list[str] = field(default_factory=list)
    # Optional valid-alias scope; only configured members can execute.
    # None = unrestricted by Skill metadata. Kept after the pre-v3.7 public
    # fields so existing positional construction does not silently change.
...
def _default_query_validator(sql: str) -> tuple[bool, str | None]:
    ...
    if is_sql_safe(sql):
        return True, None
    return False, "only SELECT/SHOW/DESCRIBE/non-ANALYZE EXPLAIN allowed"
...
def generate_skills_md(skills: dict[str, SkillMetadata], output_path: Path) -> None:
    ...
    for name in sorted(skills.keys()):
        ...
        connection_ids_str = (
            ", ".join(meta.connection_ids)
            if meta.connection_ids is not None
            else "no Skill restriction"
        )
        ...
        if len(desc) > 100:
            desc = desc[:97] + "..."
        lines.append(
            f"| {name} | {meta.type} | {meta.source} | {meta.risk} | "
            f"{databases_str} | {connection_ids_str} | {profiles_str} | "
            f"{tables_str} | {desc} |"
        )
...
def _validate_skill_identity(raw_name: Any, dir_name: str, path: Path) -> str:
    ...
    try:
        ...
        raise ValueError(f"Invalid frontmatter field 'name' in {path}: {e}") from e
...
def _parse_string_list_field(
    ...
    *,
    ...
    if isinstance(raw, str):
        ...
        raise ValueError(f"'{field_name}' must be a non-empty string or list in {path}")
...
def _parse_exact_bool(
    ...
    *,
    ...
    """Parse a policy-relevant frontmatter boolean without truthy coercion."""
    value = front.get(field_name, default)
    if type(value) is not bool:
        raise ValueError(f"'{field_name}' must be a YAML boolean in {path}")
    return value
...
def _parse_string_list(
    ...
    """Parse an optional catalog list while rejecting scalar/string drift."""
...
def _parse_category(raw: Any, path: Path) -> str:
    """Parse a declared catalog category as one non-empty string."""
    if not isinstance(raw, str) or not raw.strip():
        raise ValueError(f"'category' must be a non-empty string in {path}")
    return raw.strip()


def _parse_databases(raw: str | list | None, path: Path) -> list[str] | None:
    """
...
def _parse_connection_ids(raw: Any, path: Path) -> list[str]:
    """Parse the optional deployment-specific connection alias scope.

    The field intentionally accepts only a YAML list. A one-item list is an
    exact binding; multiple items allow deliberate Skill reuse. Omission means
    the Skill adds no connection restriction. Runtime code still resolves one
    target connection and re-applies all server-side policies.
    """
    if not isinstance(raw, list) or not raw:
        raise ValueError(f"'connection_ids' must be a non-empty list in {path}")
    ...
    for value in raw:
        if not isinstance(value, str) or not value.strip():
            raise ValueError(
                f"'connection_ids' entries must be non-empty strings in {path}"
            )
        connection_id = value.strip().lower()
        if not _CONNECTION_ID_PATTERN.fullmatch(connection_id):
            raise ValueError(
                f"Invalid connection_id {value!r} in {path}. Use 1-64 "
                "lowercase letters, digits, or underscores, starting with a letter."
            )
        if connection_id in seen:
            raise ValueError(f"Duplicate connection_id {connection_id!r} in {path}")
        seen.add(connection_id)
        normalized.append(connection_id)

    # Ordering has no routing meaning; sort for stable discovery output and to
    # make it explicit that runtime must never auto-select the first entry.
    return sorted(normalized)
...
def _parse_skill_md(path: Path, dir_name: str) -> SkillMetadata:
    ...
    Uses '---' delimiter to extract YAML frontmatter with a SafeLoader variant
    that rejects duplicate keys. No dependency on python-frontmatter library.
    ...
    try:
        front = yaml.load(parts[1], Loader=_UniqueKeySafeLoader)
    ...
    unknown_fields = set(front) - _SUPPORTED_FRONTMATTER_FIELDS
    if unknown_fields:
        unknown = ", ".join(sorted(repr(field) for field in unknown_fields))
        raise ValueError(
            f"Unsupported frontmatter field(s) {unknown} in {path}; "
            "unknown fields are rejected to prevent silent policy typos"
        )

    # Validate required fields
    for required_field in ("name", "type", "risk", "source"):
        if required_field not in front:
            raise ValueError(f"Missing required field '{required_field}' in {path}")
    ...
    return SkillMetadata(
        ...
        triggers=_parse_string_list(
            front["triggers"] if "triggers" in front else [],
            "triggers",
            path,
        ),
        version=str(front.get("version", "1.0")),
        enabled=_parse_exact_bool(front, "enabled", path, default=True),
        idempotent=_parse_exact_bool(front, "idempotent", path, default=False),
        requires_confirmation=_parse_exact_bool(
            front,
            "requires_confirmation",
            path,
            default=False,
        ...
        category=(
            _parse_category(front["category"], path) if "category" in front else None
        ),
        related_skills=_parse_string_list(
            front["related_skills"] if "related_skills" in front else [],
            "related_skills",
            path,
        ),
        databases=_parse_databases(front.get("databases"), path),
        connection_ids=(
            _parse_connection_ids(front["connection_ids"], path)
            if "connection_ids" in front
            else None
        ),
...
def _load_mutation_class(skill_name: str, mutation_path: Path) -> type:
    ...
    from sql_safety_executor.skills.mutation import ManagedMutationBase, MutationBase

    if not issubclass(mutation_class, MutationBase):
        raise TypeError(f"Skill '{skill_name}': Mutation must subclass MutationBase")
...
def _validate_param_schema(raw_params: Any, path: Path) -> dict[str, dict]:
    ...
    for param_name, constraints in raw_params.items():
        if not isinstance(param_name, str) or not param_name.strip():
            raise ValueError(f"'params' keys must be non-empty strings in {path}")
        ...
        unknown_constraints = set(constraints) - _SUPPORTED_PARAM_CONSTRAINTS
        if unknown_constraints:
            unknown = ", ".join(sorted(repr(field) for field in unknown_constraints))
            raise ValueError(
                f"Unsupported constraint(s) {unknown} for parameter "
        ...
        if "required" in constraints and type(constraints["required"]) is not bool:
            raise ValueError(
                f"Parameter '{param_name}' constraint 'required' must be a "
                f"YAML boolean in {path}"
            )

        if "description" in constraints and not isinstance(
            constraints["description"], str
        ):
            raise ValueError(
                f"Parameter '{param_name}' constraint 'description' must be a "
                f"string in {path}"
            )

        for bound_name in ("min", "max"):
            if bound_name not in constraints:
                continue
            if expected_type not in {"int", "float"}:
                raise ValueError(
                    f"Parameter '{param_name}' constraint '{bound_name}' is only "
                    f"valid for int or float parameters in {path}"
                )
            bound = constraints[bound_name]
            if expected_type == "int":
                valid_bound = type(bound) is int
            else:
                valid_bound = type(bound) in {int, float}
            if not valid_bound:
                raise ValueError(
                    f"Parameter '{param_name}' constraint '{bound_name}' must "
                    f"match numeric type '{expected_type}' in {path}"
                )

        if "min" in constraints and "max" in constraints:
            if constraints["min"] > constraints["max"]:
                raise ValueError(
                    f"Parameter '{param_name}' constraint 'min' must not exceed "
                    f"'max' in {path}"
                )
        ...
        if "enum" in constraints:
            ...
            if not isinstance(enum_values, list) or not enum_values:
                raise ValueError(
                    f"Parameter '{param_name}' constraint 'enum' must be a "
                    f"non-empty list in {path}"
                )
            for enum_value in enum_values:
                if expected_type == "int":
                    compatible = type(enum_value) is int
                elif expected_type == "float":
                    compatible = type(enum_value) in {int, float}
                elif expected_type == "bool":
                    compatible = type(enum_value) is bool
                else:
                    compatible = isinstance(enum_value, str)
                if not compatible:
                    raise ValueError(
                        f"Parameter '{param_name}' constraint 'enum' contains "
                        f"a value incompatible with type '{expected_type}' in {path}"
                    )

        validated_params[param_name] = dict(constraints)
...
class SkillCatalog:
    """One server's startup snapshot; definitions are never re-read on execution."""
    ...
    def discover(
        ...
        *,
        ...
        for entry in sorted(skills_dir.iterdir()):
            ...
            if not _is_path_within(entry, skills_dir):
                logger.warning(
                    "Skipping Skill directory outside configured trust root: %s",
                    entry.name,
                )
                continue
            skill_md_path = entry / "skill_def.md"
            if not _is_path_within(skill_md_path, entry):
                logger.warning(
                    "Skipping Skill definition outside its directory: %s", entry.name
                )
                continue
    ...
    def load_query(self, skill_name: str) -> tuple[str, dict]:
        """
    ...
    def load_mutation(self, skill_name: str, adapter, audit_logger) -> "MutationBase":
        """
    ...
    def get_skills_cache(self) -> dict[str, SkillMetadata]:
