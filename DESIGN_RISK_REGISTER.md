...
| DRR-2026-048 | Accepted | Low | 2026-08-13 | Preview-token delivery and consumption bookkeeping edges | A successful preview registers its token before best-effort audit, `ctx.info()`, and `ToolResult` construction. If a server-observable exception occurs after registration but before the result is returned, the client receives no usable token while its record occupies one bounded store entry until expiry/lazy cleanup; audit may still say preview success, and the generic adapter sanitizer can misclassify a response/logging failure as a database-query failure. This does not permit a mutation or replay because the undisclosed high-entropy token remains required. A related internal maintenance edge exists because `consume()` removes the record before the helper checks the server-generated binding hash/JSON; a future invariant regression could consume a token before the caller sets `token_consumed=True`, although the current sole issuance path makes that branch unreachable. | No immediate code change | Keep the current fail-closed behavior, short TTL, bounded capacity, and same-process atomic consume. Do not claim atomic token delivery: a network failure after the function returns is inherently ambiguous. If operational evidence or nearby lifecycle work justifies it, add a locked digest-specific `discard()` for server-observed pre-return failures, distinguish response-handling errors from database errors, and add regression coverage. Revisit the consume-result signaling only if binding/storage paths are refactored or a shared/persistent backend is introduced. | Accepted for v3.6.1 as a low-impact delivery/accounting limitation. No unauthorized write, replay, or optimistic-lock bypass is introduced; the possible orphan is bounded by store capacity and token expiry. The internal post-pop checks compare and parse data produced by the same canonical server issuance path. | Reassess if preview response failures are observed, HTTP transport becomes a primary deployment mode, or token storage/serialization gains another producer. Any cleanup must remain best-effort and must not imply that server-side code can know whether a response lost after handoff reached the client. |
